...
class TopHeadlinesNews(NewsApiBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if search_form.is_valid() and filters_form.is_valid():
            params = {
                'q': search_form.cleaned_data.get('query'),
                'country': ','.join(filters_form.cleaned_data.get('country') or []),
                'category': ','.join(filters_form.cleaned_data.get('category') or []),
...
class EverythingNews(NewsApiBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if search_form.is_valid() and filters_form.is_valid():
            params = {
                'q': search_form.cleaned_data.get('query'),
                'searchIn': ','.join(filters_form.cleaned_data.get('search_in') or []),
                'sources': ','.join(filters_form.cleaned_data.get('sources') or []),
                'from': filters_form.cleaned_data.get('_from'),
                'to': filters_form.cleaned_data.get('to'),
                'language': ','.join(filters_form.cleaned_data.get('language') or []),
            }
        else:
            params = {'q': self.default_query}
...
class GuardianNews(NewsApiBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if search_form.is_valid() and filters_form.is_valid():
            params = {
                'q': search_form.cleaned_data.get('query'),
                'section': '|'.join(filters_form.cleaned_data.get('section_or') or []),
